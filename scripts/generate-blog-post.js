...
(본문 작성 규칙)
1. **글자수 확보**: 반드시 공백 포함 **1,500자 이상**의 풍성한 분량으로 작성해. 각 섹션마다 상세한 설명과 팁을 추가해줘.
2. **가독성 최우선**: 문장은 짧게 쓰고, 2~3문장마다 문단을 나누어 빈 줄(Line Break)을 두 번 넣어서 빽빽하지 않게 작성해.
3. **시각적 요소**: 중요 키워드는 **굵게** 강조하고, 적절한 이모지를 사용하여 친근하게 작성해.
4. **구조화**: ### 소제목을 활용하여 내용을 체계적으로 나누고, 목록이나 불렛 포인트를 적극 활용해.
5. **구성**:
   - 인사 및 도입부 (상황에 공감하며 친근하게 시작)
   - ### 💡 핵심 내용 요약 (표나 목록을 활용해 한눈에 들어오게)
   - ### ✅ 상세 지원 내용 및 자격 (구체적인 조건과 예시를 들어 설명)
   - ### 📝 신청 방법 및 준비물 (어디서, 어떻게 하는지 단계별로 친절하게)
   - ### 🧐 놓치기 쉬운 꿀팁 (추가로 알면 좋은 정보 2-3가지)
   - 맺음말 (따뜻한 응원과 함께 마무리)
